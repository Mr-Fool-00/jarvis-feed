...
79. **CRITICAL: git checkout main STILL not in runbook Step 0** — This run (June 13 AM) again started in detached HEAD state (51 commits behind main). Same root cause documented in suggestions #21, #23, #26, #28 — none made it into the runbook because those suggestions were in detached HEAD commits that never pushed to origin/main. The runbook MUST have `git checkout main 2>/dev/null || true && git pull origin main --ff-only 2>/dev/null || true` as the FIRST git command in Step 0 before ANY git add/commit. Without this, every new CCR container spawns in detached HEAD and the first N commits are lost. Documenting again here so this survives a push to origin/main.

80. **agent_suggestions.md truncation — runs June 5 PM through June 12 suggestions lost** — The detached HEAD issue (#79) caused all agent suggestions from runs June 5 PM through June 12 PM to never reach origin/main. Suggestions #58-78 (covering: Fable 5 prompting, Channel Fracture architecture, fiction pipeline design, Osmani AEO framework, DW analysis wave, Glasswing monitoring, billing cross-reference, WWDC seasonal source, arxiv skills cluster gap, security research sources) are in detached HEAD only. This is a recurring data-loss pattern. Until Step 0 git checkout is fixed, assume any content written in the same run as a failed push is lost.

81. **Fable 5 privacy note should be a permanent scoring flag** — Fable 5 retains all prompts/outputs for 30 days on ALL platforms (including Max subscription), unlike Sonnet/Opus. When surfacing Fable 5 prompting guides or fiction pipeline recommendations, always include this flag. Add to INTEREST_PROFILE.md under "Output style for digest items": "For any item recommending Fable 5 use with personal/creative content, include the 30-day retention note."

82. **Add Fable 5 free window end date (June 22) as a countdown to next 3 digests** — Leo has until June 22 to test Fable 5 free on Max. The AM run on June 13 is the first to surface this. The June 14 and June 15 digests should include a brief "N days left to test Fable 5 free" reminder in the trends section. After June 22, Fable 5 becomes API-rate priced even on subscription.

83. **Channel Fracture paper (arxiv:2606.04896) should inform Jarvis state injection design** — The paper describes how split-context failures occur when memory injected at session start conflicts with mid-session tool results. For Jarvis specifically: seen.json + agent_suggestions.md + reactions.md are injected at run start (via Read tools). If Jarvis discovers a new item mid-run that contradicts the injected state, it can cause channel fracture. Mitigation: keep injected state compact and factual (IDs + scores only), not interpretive. Don't inject analysis from prior runs — re-derive it from raw IDs. This is a structural change to consider.

84. **PAT auth still failing — need Leo to check secrets** — The GITHUB_PAT in the routine prompt (github_pat_11B5AQ2UI0mukomssorV5K...) is returning "Invalid username or token" on every run. All content this run was pushed via GitHub MCP fallback. Leo should check if the PAT expired or was revoked and update the CCR environment secret. Until fixed, every run depends on the MCP fallback which has rate limits and different file-by-file semantics.
