...
58. **CRITICAL: Orphaned branch situation requires Leo decision** — This run started with git in detached HEAD 50 commits ahead of origin/main. These 50 commits represent ALL Jarvis runs from June 6 through June 19 (AM run this session). None of them share a merge-base with origin/main (the main branch sits at the June 5 AM run). Options for Leo:
    - **Option A (Safe):** Accept loss of June 6-19 orphaned chain. Current approach this run — proceed from June 5 baseline, re-discover June 6-19 highlights if worth keeping.
    - **Option B (Recover via push_files):** Use GitHub MCP push_files to reconstruct key digests and briefings from the orphaned chain onto main. Labor-intensive but preserves history.
    - **Option C (Fix the root cause):** Identify why the CCR environment keeps spawning in detached HEAD. Likely cause: each routine session is cloned fresh, applies commits, but the push fails (PAT rejected), so the next session starts from origin/main's stale SHA. A push fix (new working PAT) would stop the orphaning permanently.
    - **Immediate recommendation:** Fix the GITHUB_PAT. The current PAT in the routine prompt (`github_pat_11B5AQ2UI0...`) is rejected. Without a working PAT, every run that pushes via git will fail, and the orphaned chain problem recurs every 12 hours.

59. **PAT rotation needed — root cause of all push failures** — The GITHUB_PAT in the routine prompt has been rejected since at least the June 5 run (possibly earlier). All successful pushes since then have gone through the GitHub MCP push_files fallback (which uses a different auth token, not the PAT). But push_files doesn't push local git commits — it creates new file blobs, which creates divergence. Long-term this corrupts the git history and makes local state (seen.json, failures.log, digests) drift from origin/main. Leo needs to: (1) Rotate the PAT at github.com/settings/tokens with repo scope, (2) Update the secret in the CCR routine settings, (3) Verify with `git remote -v` + `git push` on next run.

60. **Add CwC Tokyo announcement search as permanent source** — Suggestion 57 recommended adding Tokyo announcements to the June 10 PM run specifically. The June 19 PM run found that Managed Agents (Scheduled Deployments + Vaults) from CwC Tokyo on June 10 was NOT in seen.json — a 9-day lag. Suggestion: add a persistent WebSearch for recent CwC events (`"Code with Claude" OR "CwC" announcement 2026`) to every run's discovery pass, not just specific event dates. High-value items from CwC events have been consistently delayed by 3-9 days.
